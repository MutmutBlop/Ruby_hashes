...
puts "Combien de handles contiennent un numéro ?"
number = list.select { |item| item =~ /[0123456789]/ }
puts number.length

puts "Combien de handles contiennent les 4 lettres du prénom aude à la suite ?"
aude = list.select {|s| s.include? 'aude'}
puts aude.length

puts "Combien commencent par une majuscule ?"
start_maj = list.count { |item| item =~ /^@[A-Z]/}
puts start_maj

puts "Combien contiennent une majuscule ?"
maj = list.select { |item| item =~ /[A-Z]/ }
puts maj.length

puts "Combien y a-t-il de underscore _ dans tous les pseudos confondus ?"
underscore = list.select { |item| item =~ /[_]/ }
puts underscore.length

puts "Trie la liste par ordre alphabétique."
puts list.sort_by { |word| word.downcase }

puts "Quels sont les 50 handles les plus courts de cet array ?"
short = list.min(50) {|a, b| a.length <=> b.length }
puts short
